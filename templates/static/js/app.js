...
// Tab-specific init (Data Prep needs none — its card is static).
function initTab(name) {
  if (name === "community") initCommunityTab();
  else if (name === "query") initQueryTab();
  else if (name === "batch") initBatchTab();
}
...
// Admin-only load-time check — the Data Prep tab/panel isn't shown to the user
// role, and its /api/data-prep calls are 403'd for that role.
if (document.querySelector('.tab[data-tab="data-prep"]')) checkExistingGraph();
...
// ── Init the active tab on load ──────────────────────────────
// For the user role the active tab is Query (Data Prep/Community are hidden),
// so it must initialise without a click. Runs last, after all refs are set up.
initTab(document.querySelector(".tab.active")?.dataset.tab);
