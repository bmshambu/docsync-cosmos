...
class Settings(BaseSettings):
    ...
    # ── Access (simple PoC login, two roles) ────────────────────
    # APP_PASSWORD = admin (all tabs). APP_USER_PASSWORD = tester/user
    # (Query + Batch Q&A tabs only; blocked from Data Prep & Community APIs).
    # Empty APP_PASSWORD = no login (local dev, full access). Session lasts
    # APP_SESSION_HOURS. Leave APP_USER_PASSWORD empty to have admin only.
    app_password: str = ""
